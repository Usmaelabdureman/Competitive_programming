...
def merge(arr, l, mid, r):
    # wins array to hold the number of wins each team has
    wins = [0] * (r - l + 1)
    ...
    for i in range(l, mid + 1):
        # move the second group's pointer to an element which is greater or equal to it
        while ptr <= r and arr[ptr][1] < arr[i][1]:
            ptr += 1
 
        # the total number of wins for the current element
        # is the number of elements in the second group
